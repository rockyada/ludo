            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <h5 class="text-dark font-weight-bold my-1 mr-5">
                        {{Config('constants.PLAYERS.PLAYERS_TITLES')}}
                    </h5>
                        ...
                        <div class="card card-custom card-stretch card-shadowless">
                            <div class="card-header">
                                ...
                                <div class="card-toolbar">
                                    ...
                                    <a href='{{route("$model.create")}}' class="btn btn-primary">Add New {{Config('constants.PLAYERS.PLAYERS_TITLE')}} </a>
                                </div>
                            </div>
                            <div class="card-body">
                                            ...
                                            <div class="row mb-6">
                                                <div class="col-lg-3  mb-lg-5 mb-6">
                                                    ...
                                                    <select name="is_blocked" class="form-control select2init"
                                                        value="{{ $searchVariable['is_blocked'] ?? '' }}">
                                                        <option value="">All</option>
                                                        <option value="0"
                                                            {{ isset($searchVariable['is_blocked']) && $searchVariable['is_blocked'] == 0 ? 'selected' : '' }}>
                                                            Activate</option>
                                                        <option value="1"
                                                            {{ isset($searchVariable['is_blocked']) && $searchVariable['is_blocked'] == 1 ? 'selected' : '' }}>
                                                            Block</option>
                                                    </select>
                                                </div>
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                        ...
                                        <table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
                                            <thead>
                                                <tr class="text-uppercase">
                                                    ...
                                                    <th class="{{(($sortBy == 'wallet' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'wallet' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.index',array( 'sortBy' => 'wallet',
                                                    'order' => ($sortBy == 'wallet' && $order == 'desc') ? 'asc' : 'desc',  
                                                    $query_string))}}">Wallet</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'referral_code' && $order == 'des') ? 'sorting_desc' : (($sortBy == 'referral_code' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.index',array( 'sortBy' => 'referral_code',
                                                    'order' => ($sortBy == 'referral_code' && $order == 'desc') ? 'asc' : 'desc',  
                                                    $query_string))}}">Refferal Code</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'total_win' && $order == 'des') ? 'sorting_desc' : (($sortBy == 'total_win' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.index',array( 'sortBy' => 'total_win',
                                                    'order' => ($sortBy == 'total_win' && $order == 'desc') ? 'asc' : 'desc',  
                                                    $query_string))}}">Total Game Win</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'total_lost' && $order == 'des') ? 'sorting_desc' : (($sortBy == 'total_lost' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.index',array( 'sortBy' => 'total_lost',
                                                    ...
                                                    <th class="{{(($sortBy == 'is_blocked' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'is_blocked' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                    <a href="{{route($model.'.index',array( 'sortBy' => 'is_blocked',
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">{{ $result->vplay_id ?? '' }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                           <img width="20px" src="{{Config('constants.WEBSITE_URL').'public/admin/img/global-rupeeIcon.png'}}">&nbsp;&nbsp;₹ {{ $result->wallet ?? '' }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                            <img width="30px" src="{{Config('constants.WEBSITE_URL').'public/admin/img/win.png'}}">&nbsp;&nbsp;
                                                            {{ $result->total_win ?? '' }}
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                            <img width="30px" src="{{Config('constants.WEBSITE_URL').'public/admin/img/lost.png'}}">&nbsp;&nbsp;
                                                            {{ $result->total_lost ?? ''}}
                                                        </div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-right pr-2">
                                                        @if($result->is_blocked == 0)
                                                        <a title="Click To Block" href='{{route($model.".status",array($result->id,1))}}' class="btn btn-icon btn-light btn-hover-danger btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Deactivate">
                                                        ...
                                                        <a href="{{route($model.'.changedPassword',[base64_encode($result->id)])}}" class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="Change Password">
                                                            <span class="svg-icon svg-icon-md svg-icon-primary">
